...
mod tests {
    ...
    fn renders_nested_code_list_preview() {
        ...
        s.connected = true;
        let md = "### Nested Lists with Embedded Code\n\nA response that mixes a nested ordered list with a fenced code block:\n\n1. **Parent item one**:\n   - Child item with a wrapped continuation line that must stay flush under the bullet.\n   - Another child, also with a long continuation line to exercise wrapping.\n2. **Parent item two**:\n   - Child under the second parent.\n\n```markdown\n1. Continuation Line Bullets Fixed:\n   - Previously, wrapped lines were treated as new bullets.\n```\n\nThe nested-list renderer keeps parent titles on their own line, indents children by depth, and gives fenced blocks a monospace box with a language badge.";
    ...
    fn renders_markdown_cleanup_preview() {
        ...
        let mut s = AiStreamScreen::new(1236, 1648);
        s.connected = true;
        let md = "Example of a parsed turn after markdown cleanup:\n\n### Visual Improvements to the AI Stream\n\n1. **Broken Glyph Bullets Fixed**:\n   * Bullets that previously rendered as missing-glyph squares are now drawn as crisp ink disks.\n2. **Inline Markdown Cleaning**:\n   * Raw formatting syntax (`**bold**`, `` `code` ``, `[link](url)`) is stripped and normalized into readable text.\n3. **Prompt Header**:\n   * Page one shows the prompt card; later pages shrink it to a one-line crumb.\n4. **Metadata Filtering**:\n   * System wrapper tags and timestamps are removed from the prompt.";
    ...
    fn renders_sheet_gestures_preview() {
        ...
        let mut s = AiStreamScreen::new(1236, 1648);
        s.connected = true;
        let md = "### Quick Settings Sheet Gestures\n\nThe sheet gives the AI stream screen full ownership of gestures:\n\n1. **Edge Gestures Unlocked**: the app-level gesture interceptor no longer swallows edge swipes meant for the sheet.\n2. **Simplified Triggers**: swipe up anywhere, or tap the top header (clock, prompt card).\n3. **Full Row Touch Targets**: every pixel in the three rows maps to a button:\n   - Row 1: `[ Auto (Latest) ]` / `[ Antigravity ]` / `[ Claude Code ]`\n   - Row 2: `[ < Prev Turn ]` / `[ Next Turn > ]` / `[ Live Latest ]`\n   - Row 3: `[ Poll Now ]` / `[ Clear Ghosting ]` / `[ Done ]`\n4. **Dismissing**: swipe down, tap `[ Done ]`, or tap the shaded area above the sheet.";
    ...
    fn renders_markdown_engine_preview() {
        ...
        let mut s = AiStreamScreen::new(1236, 1648);
        s.connected = true;
        let md = "### Markdown Engine\n\nThe parser is **pulldown-cmark**, driving a structured AST layout:\n\n1. **Continuation lines**: a bullet or ordered number renders only on the first line; wrapped lines indent flush underneath.\n2. **Nested lists**: sub-bullets indent by `depth * pt(12.0)` with consistent spacing.\n3. **Symbol sanitizing**: unsupported glyphs (`↑`, `↓`, `✓`, `🟢`) map to legible ASCII before shaping.\n4. **Typography**: sized headings (H1 14pt, H2 12pt, H3 10.5pt, body 8.5pt) with clean rules and margins.";
    ...
    fn renders_claude_response_preview() {
        ...
        let mut s = AiStreamScreen::new(1236, 1648);
        s.connected = true;
        let md = "Example assistant reply:\n\n## What's good\n\n- **Rendering model**: `repaginate()` precomputes page items with absolute coordinates and `draw()` walks them, so there is no layout work per frame and a revision only repaints when it changes; same-turn updates keep the current page while new turns reset to page zero, which is the discipline the paginator uses everywhere.\n- **Markdown parsing**: pulldown-cmark drives a structured AST walker, so nested lists, GFM alerts, tables, and fenced code blocks with language badges are handled structurally rather than by line heuristics; the parser is the same one used for the rest of the product.\n- **Text sanitizing**: arrows, check marks, and emoji map to ASCII before shaping, and bullets are drawn as filled disks instead of relying on font coverage, so unsupported glyphs never appear as placeholder boxes.\n- **Screen real estate**: the prompt card on page one collapses to a one-line crumb on later pages, and turn-history stepping uses negative indices to reach recent turns quickly.\n- **Source switching**: the sheet can switch between auto, antigravity, and claude sources without leaving the screen, and the choice persists for the session.\n- **Polling**: the worker reconnects with exponential backoff and only repaints when the revision changes, so a quiet session costs nothing on the panel.";
    ...
    fn renders_long_form_markdown_preview() {
        ...
        let mut s = AiStreamScreen::new(1236, 1648);
        s.connected = true;
        let md = "## Long-Form Markdown Turn\n\nA turn that exercises the full CommonMark layout pipeline: headings, nested lists, a table, a code block, and an alert.\n\n---\n\n### 1. Feature Checklist\n\n- **Heading scale and weights**: headings size hierarchically with a doubled stem for e-ink legibility.\n- **Inline code**: tokens like `repaginate()`, `Conn::new()`, and `StyledSpan` get padded boxes and clean spacing.\n- **Nested lists**: ordered items with bold numbering and child bullets that indent per depth.\n\n---\n\n### 2. Architecture Table\n\n| Component | UI Thread | Worker Thread | Channel |\n|---|---|---|---|\n| Network polling | Non-blocking | Background TCP / UDP | `mpsc::Receiver<PollerMsg>` |\n| Paginator | `repaginate()` on rev-diff | Pre-computes items | In-memory cache |\n| Keepalive | `holds_awake = true` | Persistent `Conn` | Backoff 1.5-30 s |\n\n---\n\n### 3. Worker Snippet\n\n```rust\nfn worker_loop(rx: Receiver<PollerCmd>, tx: Sender<PollerMsg>) {\n    let mut conn: Option<Conn> = None;\n    let mut fail_count: u32 = 0;\n    loop {\n        let sleep = match fail_count {\n            0 => Duration::from_millis(1400),\n            n => Duration::from_millis((1500 * (1 << n.min(5) - 1)).min(30_000)),\n        };\n        if let Ok(resp) = conn.request(\"GET\", \"/live\", &mut sink) {\n            tx.send(PollerMsg::TurnUpdate(resp));\n            fail_count = 0;\n        } else {\n            fail_count += 1;\n        }\n    }\n}\n```\n\n> [!TIP]\n> **Navigation**: tap right 35% for next page, left 35% for previous, tap the top bar or swipe up for the sheet, swipe down from the top edge for the system curtain.";
