...
- **Dictionary & flashcards.** Long-press any word for a lookup in a
  15 MB embedded dictionary: ~155k English headwords with WordNet
  glosses, frequency-ranked difficulty and CEFR level, and Russian
  translations for ~33k of them; a rule-based lemmatizer catches
  inflected forms. Star a word and it lands in an SM-2 flashcard deck
  (Again / Hard / Good / Easy) with its own home-screen trainer.
...
yb-reader/
  ...
  tools/     rendertest (the typography gallery), build_vocab (the
             dictionary builder + shipped vocab.bin), ruler/inject
             (framebuffer + synthetic-touch helpers)
