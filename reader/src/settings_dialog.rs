...
pub struct ReaderSettingsDialog {
    ...
    pub tab: usize, // 0 = Typography, 1 = Split & Crop, 2 = Contrast & Display
    #[allow(dead_code)]
    pub is_pdf: bool,
...
impl Screen for ReaderSettingsDialog {
    fn draw(&mut self, p: &mut Painter) {
        ...
        let tab_h = pt(32.0);
        let tab_w = dw / 3;
        let tabs = ["1. Text", "2. Split", "3. Display"];

        for (i, label) in tabs.iter().enumerate() {
            let tx0 = dx + i as i32 * tab_w;
            let tx1 = if i == 2 { dx + dw } else { tx0 + tab_w };
            let tr = Rect::new(tx0, dy, tx1 - tx0, tab_h);

            if self.tab == i {
                p.rect(tr, 0);
                p.text_center_in(tx0, tx1, dy + pt(21.0), 9.0, 255, label);
            } else {
                p.rect_outline_t(tr, 1, 150);
                p.text_center_in(tx0, tx1, dy + pt(21.0), 9.0, 100, label);
            }
        }
        ...
        // Apply & Read Button
        let apply_y = dy + dh - pt(34.0);
        let apply_r = Rect::new(dx + pt(10.0), apply_y, dw - pt(20.0), pt(26.0));
        p.rect(apply_r, 0);
        p.text_center_in(apply_r.x, apply_r.x + apply_r.w, apply_y + pt(17.0), 9.0, 255, "APPLY & READ");
    }
        ...
        match g {
            Gesture::Tap { x, y } => {
                ...
                if ty >= dy && ty < dy + tab_h {
                    let clicked_tab = ((tx - dx) / tab_w).clamp(0, 2) as usize;
